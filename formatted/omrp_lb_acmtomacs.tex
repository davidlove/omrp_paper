...
% Metadata Information   %%% TO BE UPDATED!
\acmVolume{9}
\acmNumber{4}
\acmArticle{39}
\acmYear{2010}
...
% Page heads
\markboth{D. Love and G. Bayraksan}{Overlapping Batches for the Assessment of Solution Quality in Stochastic Programs}
...
\begin{abstract}
Overlapping Batch Means (OBM) has long been used in simulation as a method of reusing data to generate variance estimators with asymptotically lower variance.
In this article, we apply the OBM method to stochastic programming by formulating a variant of the multiple replications procedure used for assessing solution quality.
We give conditions under which the resulting optimality gap point estimators are strongly consistent, the optimality gap interval estimators are asymptotically valid, and the OBM variance estimators for optimality gap have asymptotically lower variances relative to their nonoverlapping counterparts \cite{Meketon1984,Welch1987}.
...
\category{G.1.6}{Numerical Analysis}{Optimization}[Stochastic Programming]
\category{I.6.8}{Simulation and Modeling}{Types of Simulation}[Monte Carlo]
\category{I.6.1}{Simulation and Modeling}{Simulation Theory}

\terms{Theory, Algorithms, Measurement}

\keywords{Sample average approximation (SAA); Overlapping batch means;   Stochastic programming; Optimality gap estimation}

\acmformat{David Love and G\"{u}zin Bayraksan, 2014. Overlapping batches for the assessment of solution quality in stochastic programs.}
...
grant CMMI-1345626 and grant DMS-0602173. 

Author's addresses: D. Love, (Current address) American Express, New York, NY;
G. Bayraksan, Department of Integrated Systems Engineering, the Ohio State University, Columbus, OH; email: bayraksan.1@osu.edu.
\end{bottomstuff}
...
The Overlapping Batch Means (OBM) method, proposed by \citeN{Meketon1984}, is commonly used in simulation to obtain variance estimators having variances that are comparatively lower than their nonoverlapped counterparts. 
...
\citeN{SAH90} use overlapping estimators to estimate the variance of estimators for non-means. 
More recently, overlapping estimators based on standardized time series (as opposed to batch means) have been evaluated [\citeNP{Alexopoulos01012007};\citeyearNP{Alexopoulos2007}]; see also \citeN{Meterelliyoz_etal_12}.
To the best of our knowledge, OBM has never before been studied in a stochastic optimization context. 
In this article, we show that the overlapping batches achieve nearly the same bias and lower variances relative to their nonoverlapped counterparts when used in a stochastic optimization setting.
...
This is referred to as the Multiple Replications Procedure (MRP) and was initially discussed in \citeN{Mak1999}.   
In this article, we formulate an overlapping batches version of MRP.
...
We assume that the candidate solution $\xh$ is given as input to our procedures, and hence fixed, throughout the article. 
...
\]
due to the inequality $\e{\zs_m} \leq \zs$ \cite{Mak1999,norkin_pflug_ruszczynski_98}.
...
As mentioned above, to enable statistical inference, the MRP of \citeN{Mak1999} generates $k$ independent estimators, each using sample size $m$ ($k$ nonoverlapping batches of size $m$), and averages them to obtain a point estimator.  
...
It is also easy to implement; thus, MRP has been applied to a wide variety of problems, including finance, supply chain network design, etc.; see, e.g., \citeN{bertocchi_etal_99,janjarassuk_linderoth_08,santoso_ahmed_etal_05}.  
...
\begin{enumerate}
\item We formulate an overlapping batch means variant of MRP, where the degree of overlap can be chosen by the user.  
...
\item We provide conditions under which the resulting optimality gap estimators exhibit reduced variance when compared to their counterparts obtained via nonoverlapping batches.
...
\item We examine small-sample behavior of the estimators through numerical examples. 
...
\item As the degree of overlap increases, the number of batches---and optimization problems solved---increases because we need to solve a sampling problem per batch. 
...
An earlier version of the article appeared in \citeN{love2011overlapping}. 
Most of the theoretical results in the current version of the article are new, including almost sure convergence and asymptotically lower variances of the overlapping variance estimators, and asymptotic validity of the interval estimators. 
...
To overcome this difficulty, various variance estimators have been proposed in the simulation literature; see, e.g., \citeN{law_15}.  
...
Under appropriate conditions, $n\varhat(\ybb)$ is a consistent estimator of $\sigma^2$ \cite{alexo_enc_06}.
...
In this article, we use the {\it batch nonoverlap parameter} $1 \leq \gamma \leq m$ to denote how much neighboring batches do {\it not} overlap.  
For instance, $\gamma = m$ corresponds to the case of nonoverlapping batches and $\gamma = 1$ corresponds to the maximally overlapping case of \citeN{Meketon1984}.  
...
With $\gamma=1$ (the maximally overlapping case), the denominator in (\ref{eq:variance_gamma}) is slightly different than the one in  \citeN{Meketon1984}.
This slightly different denominator makes $\vto{\ybb}$ an unbiased estimator for i.i.d.\ data for all finite $m$ and $n$, with the same asymptotic benefits of nearly the same bias and lower variance relative to its counterpart obtained via nonoverlapping batches \cite{Song1992}. 
...
Damerdji~\shortcite{damerdji1994strong,damerdji1995mean} studied batching structures that guarantee almost sure and mean-square consistency of $\vartg{\ybb}$ in the nonoverlapping and maximally overlapping cases. 
Under appropriate conditions, the results of \citeN{damerdji1995mean}, along with those of \citeN{Meketon1984} and \citeN{Welch1987}, establish that 
\begin{equation}
	\label{eq:varvarred}
	\frac{n}{m}\var{n \vartg{\ybb}} \rightarrow \beta_{\gammab} \sigma^4
\end{equation}
as the batch size $m$ and then the number of batches $n/m$ tend to infinity; see also \citeN{Alexopoulos2007}. 
...
\begin{equation} 
	\label{eq:var_reduct_formula}
	\gammab \left( 1 + 2 \sum_{j=1}^{\cogb-1} (1-j\gammab)^2 \right),
\end{equation}
where $\lceil \cdot \rceil$ denotes the smallest integer that is greater than or equal to its argument \cite{Welch1987}.
...
This is shown empirically in \citeN{Bayraksan2006}, which also investigates additional methods for using a smaller number of replications (e.g., 1 or 2) with an alternative variance estimator to compute a CI; see also \citeN{stockbridge_bayraksan_13}. 
\citeN{bayraksan_morton_09}, \citeN{partani2006jackknife}, and \citeN{partani_07} discuss variations of MRP aimed to reduce bias and variance.
...
\citeN{shapiro2000rate} establish exponential rates of convergence for a class of (\ref{eq:sto_prog}), which include two-stage stochastic linear programs with recourse when the support of $\xit$ is finite.   
\citeN{kleywegt2002sample} show that Assumption A3 can be satisfied for discrete stochastic programs (i.e., when $X$ is a finite set) under the condition that the moment generating function of $f(\xs,\xit) - f(\x,\xit)$ for $\x \in X \backslash \{\xs\}$ is finite valued on $\R$.  
...
Strong consistency of the variance estimator $n_l VD_l$ has been studied in \citeN{damerdji1994strong} for the cases of nonoverlapping and maximally overlapping batches.  
Under the additional moment assumption
\begin{describe}{{\bf A2$^\prime$\/}}
	\item[A2$^\prime$]  $\exists \epsilon > 0$ such that $\sup_{\x \in X} \e{f(\x,\xi)^{4+\epsilon}} < \infty$,
\end{describe}
for a sampling scheme of $m_l = \lfloor n_l^r \rfloor$, Corollaries 3.4 and 4.2 of \citeN{damerdji1994strong} show that $n_l VD_l$ is consistent if $\tfrac{2}{4+\epsilon} < r < 1$ for nonoverlapping batches and $\tfrac{2}{4+\epsilon} < r < \tfrac{1}{2}$ for maximally overlapping batches. 
...
\citeN{damerdji1995mean} shows that mean-square convergence is guaranteed by $\tfrac{1}{2} + \tfrac{1}{4+\epsilon} < r < 1$ for both nonoverlapping and maximally overlapping batches under the batching structure $m_l = \lfloor n_l^r \rfloor$ and $(4+\epsilon)$th-finite-moments Assumption A2$^\prime$.  
...
The instance of the newsvendor problem can be found in \citeN{Bayraksan2006}, except here we used a discrete uniform demand distribution.
The second is a capacity expansion planning problem, denoted CEP1, with random demand vector $\xit$ of dimension 3, with 216 total realizations \cite{higle1996}.
The third is a stochastic knapsack problem denoted 10D, with 10 stochastic parameters and 10 binary decision variables \cite{kleywegt2002sample}.
The final problem is a stochastic vehicle routing problem, denoted DB1, with 46 stochastic parameters and a total of $4.5 \times 10^{25}$ realizations \cite{donohue1995upper}.

\begin{table}
\tbl{Test problem characteristics\label{tb:test_problems}}{
\begin{tabular}{lcccc}
\hline
...
Furthermore, 10D has a high condition number \cite{kleywegt2002sample}, making it unlikely that the optimal solution $\xs$ is found when using a sampling solution, thus demonstrating the behavior of OMRP when the bias of the gap is significant.
...
The candidate solutions used for NVD and CEP1 and their characteristics can be found in \citeN{Bayraksan2006}.
...
The problems CEP1 and DB1 are two-stage stochastic linear programs with recourse and were solved with the regularized decomposition algorithm of \citeN{ruszczynski1986regularized}, using the accelerated version of the algorithm implemented in C++ \cite{ruszczynski1997accelerating}. 
10D is a stochastic knapsack problem and was solved using dynamic programming.  
In all cases, we used the Mersenne Twister algorithm of \citeN{matsumoto_nishimura_98} to generate the random samples, using an implementation in C++ \cite{wagner_twister_09}.
...
The solid lines in these figures show the theoretical relative decreases in variance from \citeN{Welch1987} given by (\ref{eq:var_reduct_formula}). 
...
\begin{figure}[htb!]
	...
	\subfloat[][NVD]{
	   \label{fig:nv_varvar}
	   \includegraphics[width=.5\textwidth]{nv_varvar_discrete}}
	\subfloat[][CEP1]{
	   \label{fig:cep1_varvar}
	   \includegraphics[width=.5\textwidth]{cep1_varvar}}
	\\
	\subfloat[][10D]{
	   \label{fig:10d_varvar}
	   \includegraphics[width=.5\textwidth]{10D_varvar}}
	\subfloat[][DB1]{
	   \label{fig:db1_varvar}
	   \includegraphics[width=.5\textwidth]{db1_varvar}}
	\caption{
...
\begin{figure}[htb!]
	...
	\subfloat[][Expected Variance]{
	   \label{fig:nv_avgvar}
	   \includegraphics[width=.5\textwidth]{nv_avgvar_discrete}}
	\subfloat[][Coverage Probability]{
	   \label{fig:nv_cover}
	   \includegraphics[width=.5\textwidth]{nv_cover_discrete}}
	\caption{ 
		Estimates of
		(a) $\e{nVG}$ and 
...
The coverage probabilities from applying the (nonoverlapping) MRP algorithm to these problems presented in \citeN{Bayraksan2006} agree with our results: for the newsvendor problem, coverage probability drops as $m$ increases and the coverage probability of CEP1 (not shown for brevity) remains fairly constant around the desired value of $90\%$. 
...
\begin{figure}[thb!]
	...
	\subfloat[][Theoretical]{
	   ...
	   \includegraphics[width=.49\textwidth]{comp_efficiency_theory}}
	\subfloat[][Empirical for NVD]{
	   \label{fig:efficiency_nv}
	   \includegraphics[width=.51\textwidth]{nv_comp_efficiency}}
		\caption{
...
\begin{figure}[htb!]
	...
	\subfloat[][Theoretical]{
	   ...
	   \includegraphics[width=.51\textwidth]{work_comparison}}
	\subfloat[][Empirical for NVD]{
	   \label{fig:comp_work_comparison_nv}
	   \includegraphics[width=.48\textwidth]{nv_work_comparison2}}
		\caption{
...
We have applied the overlapping batch means method from simulation output analysis \cite{Meketon1984,Song1992,Welch1987} to the problem of assessing solution quality in stochastic programming, in particular, to Monte Carlo simulation-based estimators of optimality gaps \cite{Mak1999}. 
...
The authors are grateful to the associate editor and two anonymous referees for valuable suggestions that significantly improved both the content and the presentation of the article. 
...
% History dates
\received{October 2013}{July 2014}{December 2014}
