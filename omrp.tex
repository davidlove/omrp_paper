...
We define its quality by its optimality gap $\e{f(\xh,\xit)} - \zs$. 
...
We assume this solution is given. 
...
To enable statistical inference, the \emph{Multiple Replications Procedure} (MRP) of Mak et.\ al.\ \citep{Mak1999} generates $k$ independent estimators of $G_m(\xh)$, each using sample size $m$ and averages them to obtain a point estimator.  
To form confidence intervals, the sample variance of these estimators is calculated.  
This is essentially a nonoverlapping batch means type estimator ($k$ batches of size $m$) commonly used in simulation \citep{law_07}. 
...
The degrees of freedom in (\ref{eq:overlap_variance_def}) makes $\vart{\ybb}$ an unbiased estimator for iid data for all finite $m$ and $n$ with the same asymptotic benefits \citep{Song1992}.  
...
Welch \citep{Welch1987} established the relationship between OBM and spectral estimators and also considered partial overlap, see also \citep{Song1992}.  
...
This is shown empirically in Bayraksan and Morton \citep{Bayraksan2006}, which also suggests additional methods for using a smaller number of replications (e.g., 1 or 2) with an alternative variance estimator to compute a confidence interval.  
...
It is also easy to implement, thus, has been applied to a variety of problems, see e.g., \citep{bertocchi_etal_99,janjarassuk_linderoth_08,santoso_ahmed_etal_05}.  
