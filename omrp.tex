...
\begin{abstract}
The method of Overlapping Batch Means (OBM) has long been used in simulation as a method of reusing data to generate estimators with asymptotically lower variance.
...
Computational results for several test problems are presented, examining the small-sample behavior of the overlapping batches method.\smallskip
...
We wish to assess the quality of a candidate solution $\xh \in X$, which may have been found in any way, of a stochastic program of the form
...
The quality of the candidate solution $\xh$ is defined by its optimality gap $\e{f(\xh,\xit)} - \zs$.
Assessing solution quality is important in practice since (\ref{eq:sto_prog}) typically cannot be solved exactly, so one only has an approximate solution $\xh$ without verification of its quality.
...
In stochastic programming, an approximation of \ref{eq:sto_prog} is often generated by sampling from the distribution of $\xi$.
...
This motivates estimating the upper bound on the optimality gap by using Monte Carlo sampling.
Given a sample size $m$, an upper bound on the optimality gap can be obtained by $\e{f(\xh,\xit)} - \zs \leq \e{f(\xh,\xit)} - \e{\zs_m}$ due to the inequality $\e{\zs_m} \leq \zs$.
...
The first test problem is a newsvendor problem with demand distributed uniformly on $\{0, 0.0001, 0.0002, \dots, 10\}$, the second is a capacity expansion planning problem, denoted CEP1, with random demand vector $\xit$ of dimension 3, with 216 total realizations, the third is a stochastic knapsack problem denoted 10D, with 10 stochastic parameters and 10 binary decision variables, and the final is a stochastic vehicle routing problem denoted DB1, with 46 stochastic parameters and a total of $4.5 \cdot 10^{25}$ realizations.
Descriptions of CEP1, 10D, and DB1 can be found in \cite{higle1996}, \cite{kleywegt2002sample}, and \cite{donohue1995upper}, respectively.
...
CEP1 and DB1 were solved with the regularized decomposition algorithm of \cite{ruszczynski1986regularized}, using the accelerated version of the algorithm implemented in C++ \cite{ruszczynski1997accelerating}. 
The newsvendor problem was solved using the quicksort algorithm in C++, and 10D was solved using dynamic programming.  
In all cases, we used the Mersenne Twister algorithm to generate random samples \cite{wagner_twister_09}.
