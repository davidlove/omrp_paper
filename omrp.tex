...
\begin{proof} Let $E^C$ denote the complement of event $E$. Note that
	\begin{align}
...
\begin{proof}
	For simplicity of exposition, we drop the subscript $l$ and the $\frac{n_l}{m_l}$ for now and add them in later.  
        Further, we use abbreviations $n_b = \nbl$ and $df = \left(\tfrac{n_l}{m_l} - 1\right) \left(\nbl\right)$ to denote the number of batches and the denominator term in $VG$ and $VD$ in the variably overlapping batches, respectively. Note that
	\begin{align}
	...
	\begin{align*}
		& \left( \frac{n}{df} \right)^2 n_b \e{ \sum_{1}^{n_b} \left[ (\gb_j - \gbb)^2 - (\db_j - \dbb)^2 \right]^2 } \\
		 =  & \left( \frac{n}{df} \right)^2 n_b \e{\left[ \sum_{j=1}^{\left\lceil \gammab^{-1} \right\rceil} \left[ A_j \right]^2 \
...
In this section, we apply OMRP to several test problems and examine its small-sample behavior. 
We begin our discussion by introducing the test problems used for numerically evaluating OMRP in \S \ref{ssec:test}, followed by our experimental setup in \S \ref{ssec:expS}. Then, we present and discuss our numerical experiments in \S \ref{ssec:results}. 
...
This feature shows the behavior of $VG$ when the gap size is determined mostly by sampling error, rather than bias.
...
We used the candidate solution (0, 0, 1, 1, 1, 0, 1, 1, 0, 1) for 10D with $\mu_{\xh} = 158.579$ and the candidate solution (11, 14, 8, 11, 7) with an estimated $\mu_{\xh} = 1.08$ for DB1. 
Because the optimal solution of DB1 is unknown, a Latin hypercube sample of size 50,000 was used to estimate $\mu_{\xh}$, which was then used to estimate the coverage probability of the OMRP CIs for this test problem below.
...
We also set $\alpha=0.10$ formed 90\% confidence intervals on the optimality gap using the ORMP estimators. 
...
10D is a stochastic knapsack problem and was solved efficiently using dynamic programming.  
...
Even with the relatively small sample sizes, compared to the size of this problem, overlapping appears to achieve some variance reduction. 
...
Coverage probability for 10D and DB1 (not shown) were very high, more than $0.99$.  
