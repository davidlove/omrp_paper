...
We begin our discussion by introducing the test problems used for numerically evaluating OMRP in \S \ref{ssec:test}, followed by experimental setup in \S \ref{ssec:expS}. Then, we present and discuss our numerical experiments in \S \ref{ssec:results}. 
...
All test problems are two-stage stochastic programs with recourse and were selected to study the behavior of OMRP under a variety of different situations, as we will shortly discuss below.
...
The final problem is a stochastic vehicle routing problem denoted DB1, with 46 stochastic parameters and a total of $4.5 \cdot 10^{25}$ realizations \citep{donohue1995upper}.
...
The problems CEP1 and DB1 are two-stage stochastic linear programs with recourse and were solved with the regularized decomposition algorithm of \citep{ruszczynski1986regularized}, using the accelerated version of the algorithm implemented in C++ \citep{ruszczynski1997accelerating}. 
10D is a stochastic integer program and was solved using dynamic programming.  
