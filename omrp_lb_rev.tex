...
In this setting, computational efficiency may be defined as $[T(\gammab)\var{VG}]$ where $T(\gammab)$ is the computational time.
Because variance reduction is known from (\ref{eq:var_reduct_formula}) we need only examine the computational time, which can be decomposed as $T(\gammab) = t(\gammab)\left(\nb\right)$ where $t(\gammab)$ is the average computation time per batch.
Warm-starting an algorithm may be used to decrease the batch solution time for $\gammab < 1$, leading to a possible increase in computational efficiency.

To investigate this, we assumed that computation time per batch can be approximated as $t(\gammab) \propto \gammab^p$ for some $p \geq 0$.
...
At $p=1$, i.e., for problems where computational time is proportional to the number of samples replaced in the batch, computational efficiency is maximized with maximally overlapping batches.
...
The measured computational efficiency is compared to the theoretical result in Figure \ref{fig:efficiency_nv}.
...
We can see in Figure \ref{fig:efficiency} that for some values of $p$ efficiency gian is nearly flat over a region of $\gammab$.
...
Again assuming average time per batch can be represented as $T(\gammab) = \gammab^p$, Figure \ref{fig:comp_work_comparison} compares the increase in computational work for OMRP for several values of $p$ with the work increase for MRP to get an equivalent decrease in variance.
