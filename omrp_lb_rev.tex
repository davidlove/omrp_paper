...
We assume that the candidate solution $\xh$ is given as input to our procedures, and hence fixed, throughout this paper. 
...
We note several differences in this setting compared to the classical simulation setting.  
...
An optimality gap estimator not only has a sample mean $(\frac{1}{m} \sum_{i=1}^m f(\xh,\xiti))$ but also a minimized sample mean $(\zs_m)$.  
...
In this paper, we consider a parametrized degree of overlap, where the degree of overlap is at the user's discretion, and examine this both theoretically and empirically. 
% THIS LAST SENTENCE NEEDS TO CHANGE! 
...
% the first batch contains observations $y^1, y^2, \dots, y^m$ and the second batch contains observations $y^{m+1}, y^{m+2}, \dots, y^{2m}$.  
...
%formed using the batch nonoverlap parameter $1 \leq \gamma < m$ 
...
Under appropriate conditions, these results, along with those of \cite{Meketon1984} and \cite{Welch1987}, establish that 
\begin{equation}
\label{eq:varvarred}
\frac{n}{m}\var{n \vartg{\ybb}} \rightarrow \beta_{\gammab} \sigma^4
\end{equation}
as the batch size $m$ and the number of batches $n/m$ tend to infinity. 
...
When only half of the observations overlap ($\gammab = 1/2$ or $\gamma=m/4$), for instance, the variance is 75\% of the nonoverlapping counterpart.


A $(1-\alpha)$-level approximate CI on the mean can be formed using overlapping batches by $\ybb \pm t_{d_{\gamma}(k-1),\alpha/2}$ $\sqrt{\vartg{\ybb}}$, where the degrees of freedom increase $d_{\gamma}$ is the multiplicative inverse of (\ref{eq:var_reduct_formula}) \citep{Welch1987}.  
If $\ogb$ is an integer, $d_{\gamma} = \frac{3}{2+\gammab^2}$.  For the maximally-overlapping batch means, the degrees of freedom is $d_{\gamma} = \frac{3}{2}$.
...
The results on overlapping batch means like (\ref{eq:varvarred}) occur in the limit as $n, m, k=n/m \rightarrow \infty$.  
...
\begin{proof}
	...
	\begin{align*}
		& \left( \frac{n}{dt} \right)^2 n_b \e{ \sum_{j=1}^{n_b} \left[ (\gb_j - \gbb)^2 - (\db_j - \dbb)^2 \right]^2 } \\
		 =  & \left( \frac{n}{dt} \right)^2 n_b \e{\sum_{j=1}^{\left\lceil \gammab^{-1} \right\rceil} \left( A_j \right)^2 \
		 + \sum_{j=\left\lceil \gammab^{-1} \right\rceil+1}^{n_b-\left\lceil \gammab^{-1} \right\rceil} \left( A_j \right)^2 \
		 + \sum_{j=n_b-\left\lceil \gammab^{-1} \right\rceil+1}^{n_b} \left( A_j \right)^2 
		} \\
...
Theorem \ref{thm:varvar_conv} gives conditions under which the same asymptotic lower variances are achieved for the OMRP optimality gap variance estimator relative to its nonoverlapping counterpart MRP. 
...
The variance reduction in DB1 can be less than the asymptotic amount %for the small sample sizes used in these experiments. 
because DB1 has not yet been solved to optimality due its size; therefore, it may lack a unique optimal solution. 
%, violating assumption A3. (?)
...
\begin{figure}[htb!]
	...
	\subfloat[][]{
	   ...
	   \includegraphics[width=.5\textwidth]{images/comp_efficiency_theory}}
	\subfloat[][]{
	   \label{fig:efficiency_nv}
	   \includegraphics[width=.5\textwidth]{images/nv_comp_efficiency}}
		\caption{
		Theoretical computational efficiency results for overlapping batches (left) and a comparison with measured computational efficiency for the Newsvendor problem using insertion sort (right).
		Both graphs are scaled so that nonoverlapping batches produces a computational efficiency of 1.
		}
...
In this setting, computational efficiency is defined as $[T(\gammab)\var{VG}]^{-1}$, where $T(\gammab)$ is the computational time spent to generate $VG$ for a given value of $\gammab$.
Because the variance of overlapping estimator relative to the nonoverlapping (MRP) variant is known from (\ref{eq:var_reduct_formula}), we need only examine the computational time, which can be decomposed as $T(\gammab) = t(\gammab)\left(\nb\right)$, where $t(\gammab)$ is the average computation time per batch, multiplied by the number of batches.
Warm-starting an algorithm may be used to decrease the computation time per batch $t(\gammab)$ for $\gammab < 1$, leading to a possible increase in computational efficiency. 
...
Results for various values of $p$ are shown in Figure \ref{fig:efficiency_theory}. 
This figure is constructed using $m = 1000$, but the results do not change substantially with different values of $m$.
...
%All theoretical curves in this section were computed using $m = 1000$, but the results do not change substantially with increasing batch size.
%Newsvendor computations used $m = 1000$ with $10,000$ replications.

We tested this theoretical relationship using the newsvendor problem solved with insertion sort.
Insertion sort is an efficient sorting algorithm when the values are already almost sorted. 
Therefore, as overlapping increases, insertion sort results in a decreased solution time per batch for the NVD problem. 
To test computational efficiency, we used $m = 1000$ with $10,000$ replications.
The average computation time per batch was found empirically to be well approximated ($R^2 = 0.994$) by power law with $p = 0.71$.
The empirically measured computational efficiency is compared to the theoretical result in Figure \ref{fig:efficiency_nv}, where the theoretical computational efficiency, shown in solid line, is repeated from Figure \ref{fig:efficiency_theory}.
...
For example, in the case of NVD, we recommend the use of value $\gammab=0.8$ as this value provides a high computational efficiency with minimal overall computation time across values of $\gammab$ with high efficiency. 

\begin{figure}[htb!]
	\centering
	\subfloat[][]{
	   \label{fig:comp_work_comparison_theory}
	   \includegraphics[width=.5\textwidth]{images/work_comparison}}
	\subfloat[][]{
	   \label{fig:comp_work_comparison_nv}
	   \includegraphics[width=.5\textwidth]{images/nv_work_comparison}}
		\caption{
		(Left) A theoretical comparison of the computational work required to get equivalent variance reduction between OMRP and MRP when the number of batches are increased.
		(Right) Measured computational work for NVD compared with the theoretical results.
		}
\label{fig:comp_work_comparison}
\end{figure}

A final question on computational efficiency would be whether OMRP is more efficient than nonoverlapping MRP with a larger number of batches.
When generating observations is cheap, one could potentially increase the number of batches in MRP to obtain estimators with lower variances. 
...
Continuing on in this way for various values of $\gammab$, we obtain the solid black curve in Figure \ref{fig:comp_work_comparison} using $m=1000$. 
This curve shows the increase in computational effort due to an increased number of batches in MRP in order to obtain the same variance level for OMRP across $\gammab$.
...
Again, assuming average computation time per batch can be represented as $T(\gammab) \propto \gammab^p$, Figure \ref{fig:comp_work_comparison} compares the increase in computational work for OMRP for several values of $p$ with the work increase for MRP to get an equivalent decrease in variance.
...
With these considerations, for situations where $p > 0$ but its size cannot be readily measured or estimated, we recommend selecting a large value of $\gammab$ such as $0.9$.
...
We have provided conditions under which the resulting point estimator of the optimality gap and its associated variance estimator are consistent, the same variance reduction can be achieved, and asymptotically valid CIs can be formed. 
