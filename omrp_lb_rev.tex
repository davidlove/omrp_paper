...
%This paper applies OBM to assessing solution quality in stochastic optimization. 
%Specifically, an overlapping batch means variant of MRP is formulated, where the degree of overlap can be chosen by the user. 
...
At the same time, variances of the estimators are lowered, and when warmstarting is available, solution time per batch can decrease, resulting in efficiency gains. 
We explore this trade-off between computation time and lower variances.
% and efficiency gains. 
%We explore the overall computation time versus lower variances and efficiency gains trade-off. 
Our results indicate that when warmstarting is effective, overlapping is computationally more efficient relative to estimators formed via nonoverlapping batches.
%Our results indicate that overlapping can indeed increase computational efficiency, especially when warmstarting is effective. 
...
% compared to its earlier conference proceedings version. 
...
In the overlapping batches of \citet{Meketon1984}, the first batch is as before, the second batch now consists of $y^2, y^3, \dots, y^{m+1}$, the third batch $y^3, y^4, \dots, y^{m+2}$, and so on.  
...
It is possible to vary the degree of overlap between neighboring batches. 
...
makes $\vto{\ybb}$ an unbiased estimator for i.i.d.\ data for all finite $m$ and $n$ with the same asymptotic benefits of nearly the same bias and lower variances relative to its counterpart obtained via nonoverlapping batches \citep{Song1992}. 
...
%If $\ogb$ is an integer, $d_{\gammab} = \frac{3}{2+\gammab^2}$.  
...
The increase in computation time can be alleviated in two ways: (i) by increasing $\gamma$ to partially overlap the batches, resulting in a smaller number of batches, and (ii) using warmstarting to quickly solve the sampling approximations with overlapping samples. 
%We use the batch nonoverlap parameter $\gamma$ as before to allow for varying degrees of overlap in OMRP. 
We examine the effect of warmstarting and any resulting computational efficiency gains in \S \ref{ssec:compeff}. 
...
%Second, in many solution methods, warmstarting can be used to quickly solve the sampling approximations with overlapping samples, considerably reducing solution time.
...
%In our computational results, however, we did not observe any negative values of $\gbb_l$. 
...
Then, we provide conditions under which the asymptotically lower variances are observed for the OMRP variance estimator relative to nonoverlapping MRP estimator.  
...
With i.i.d.\ sampling, $\e{\db_{lj}}=\e{\dbb_l}=\mu_{\xh}, \ \forall l,j$ and by (\ref{eq:varvarred}), $\frac{n_l}{m_l} \var{n_l VD_l} \rightarrow \beta_{\gammab} \sigma^4_{\xh}$ as $l \rightarrow \infty$ under appropriate conditions. 
...
The overlapping variance estimator for stochastic programs displays the same desirable properties, detailed in \S \ref{subsec:conv} and \S \ref{ssec:var_reduct}, as in simulation, but we must still check that it results in a valid CI.  
...
The variance of OMRP variance estimators in DB1 can be less than the asymptotic amount established by Theorem \ref{thm:varvar_conv}. %for the small sample sizes used in these experiments. 
...
Warmstarting an algorithm may be used to decrease the computation time per batch for $\gammab < 1$, leading to a possible increase in computational efficiency. 
...
%		(graphs are scaled so that nonoverlapping batches produces a computational efficiency of 1)
		}
...
\begin{figure}[htb!]
	...
	\subfloat[][Empirical for NVD]{
	   ...
	   \includegraphics[width=.49\textwidth]{images/nv_work_comparison}}
		\caption{
...
We recommend that users perform preliminary computations to determine how computational time per batch changes as $\gammab$ changes when warmstarting is available. 
...
Our results indicate that when warmstarting is effective, efficiency can be observed across a wide range of overlapping degrees, and when warmstarting is less effective, efficiency increases for only small degrees of overlap. 
