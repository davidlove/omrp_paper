...
This is because we want to be as close to a standard overall mean estimator (e.g., $\ybb=\frac{1}{n}\sum_{i=1}^{n} y^i$) as possible.
With this definition of $\gbb_l$, when $\xs_j= \xs$ for all $j= 1, 2, \dots, \nbl$, we have $\gbb_l$ equal to its nonoptimized counterpart given in (\ref{eq:dbb}). 
This fact is key in establishing the theoretical results presented in the next section. 
We note, however, that this choice of $\gbb_l$ may not be guaranteed to be non-negative.
In that case, $\max\{\gbb_l,0\}$ can be used in practice. 
